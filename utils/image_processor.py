...
from schemas.photo import LocalPhotoDetail
...
def get_file_path(dest_dir: str, filename: str, user: User) -> str:
    ...
    user_dir = os.path.join(dest_dir, f"{username}_id_{user.id}")
    # Ensure the user-specific subdirectory exists
    Path(user_dir).mkdir(parents=True, exist_ok=True)
    return os.path.join(user_dir, filename)


def validate_image_file(file: UploadFile | str) -> Tuple[bool, Optional[str]]:
    """
...
async def save_uploaded_image_on_local(file: UploadFile, user: User) -> LocalPhotoDetail:
    """
