...
def register_user_service(user_data: CreateUser, db: Session) -> User | None:
...
def authenticate_user_service(email: str, password: str, db: Session) -> User | None:
...
def get_current_active_user_service(user_id: int, db: Session = Depends(get_db)) -> User | None:
