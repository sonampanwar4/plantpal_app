...
from services.ai_bot_service import clear_user_session_service
...
async def register_profile(
    ...
    try:
        ...
        register_user_service(user_data, db)
        ...
        return templates.TemplateResponse("register_profile.html",
...
async def login(
    ...
    """Handle user login."""
    user = authenticate_user_service(form_data.email, form_data.password, db)
    if not user:
    ...
    response.set_cookie("message", f"Login successful! Welcome, {user.full_name}!", max_age=5, path="/")
    response.set_cookie("message_type", "success", max_age=5, path="/")
    return response
...
async def logout(request: Request, db: Session = Depends(get_db)):
    """Handle user logout."""
    # Clear user session
    user = get_current_user_service(request, db)
    if user:
        clear_user_session_service(db, user)
